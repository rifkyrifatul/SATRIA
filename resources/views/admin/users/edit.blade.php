...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-rose-200 shadow-sm mt-6 overflow-hidden">
        <div class="p-6 sm:p-8 border-l-4 border-rose-500">
            ...
            <p class="text-sm text-slate-600 dark:text-slate-400 mb-6 font-medium">Sebagai Super Admin, Anda tidak diizinkan membuat kata sandi manual untuk pengguna lain demi menjaga privasi. Gunakan tombol di bawah ini untuk mengatur ulang kata sandi pengguna ini ke default <span class="font-bold text-rose-600 bg-rose-50 px-2 py-0.5 rounded border border-rose-100">12345678</span>.</p>
            
            <form method="POST" action="{{ route('super_admin.users.reset_password', $user) }}" onsubmit="window.confirmAction(event, 'Peringatan: Kata sandi akan di-reset menjadi \'12345678\'. Anda yakin ingin melanjutkan?', 'Ya, Reset Sandi!', '#f59e0b', 'warning');">
                @csrf
                <button type="submit" 
