...
@section('title', 'EDIT ALUR PENGAJUAN')
...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center gap-4 mb-6">
        ...
        <div>
            <h1 class="text-2xl font-extrabold text-slate-900 dark:text-white">Pembaruan Alur Pengajuan</h1>
            <p class="text-sm text-slate-500 mt-1">Perbarui informasi nama atau alur verifikasi birokrasi.</p>
        </div>
    </div>

    <form action="{{ route((Auth::user()->role === 'super_admin' ? 'super_admin' : 'admin') . '.categories.update', $category) }}" method="POST" class="bg-white dark:bg-slate-800 rounded-xl border-2 border-slate-300 dark:border-slate-600 shadow-sm p-6 sm:p-8">
        @csrf
        @method('PUT')
        ...
        <div x-data="{ reviewType: '{{ old('review_type', $category->review_type) }}' }" class="mb-8">
