...
@section('title', 'UBAH TEMPLATE SURAT')
...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4 mb-6">
        ...
        <div>
            <h1 class="text-2xl font-extrabold text-slate-900 dark:text-white">Pembaruan Data Template</h1>
            <p class="text-sm text-slate-500 mt-1">Perbarui informasi atau ganti file template.</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border-2 border-slate-300 dark:border-slate-600 shadow-sm overflow-hidden">
        <form action="{{ route("{$routePrefix}.letter_templates.update", $letterTemplate) }}" method="POST" enctype="multipart/form-data" class="p-6 sm:p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3"
                          class="w-full rounded-xl border-slate-300 dark:border-slate-600 focus:border-indigo-500 focus:ring-indigo-500 text-sm py-2.5 px-4 bg-slate-50 dark:bg-slate-900/50 focus:bg-white transition-colors">{{ old('description', $letterTemplate->description) }}</textarea>
                @error('description')
                    <p class="mt-2 text-xs text-rose-600 font-bold flex items-center gap-1">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                        {{ $message }}
            ...
            <div>
                <label for="file" class="block text-sm font-bold text-slate-800 dark:text-slate-200 mb-2">Ganti File Document <span class="text-[10px] uppercase tracking-wider text-slate-400 font-bold ml-1">(Biarkan kosong jika tak ingin diganti)</span></label>
                    ...
                    <div class="space-y-1 text-center">
                        ...
                        <div class="flex text-sm text-slate-600 dark:text-slate-400 justify-center">
                            <label for="file" class="relative cursor-pointer font-bold text-indigo-600 hover:text-indigo-700">
                                ...
                                <input id="file" name="file" type="file" class="sr-only" accept=".doc,.docx,.pdf,.xls,.xlsx,.jpg,.jpeg,.png">
                            </label>
                        </div>
