...
<div class="max-w-7xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-extrabold text-slate-900 dark:text-white flex items-center gap-2">
                ...
                Monitoring Posisi Surat & Tracking Alur
            </h1>
            <p class="text-sm font-medium text-slate-500 mt-1">Pantau seluruh posisi tahapan persetujuan dan riwayat alur surat pengajuan di dalam sistem.</p>
        </div>
    </div>

    {{-- ── Stat Counters ── --}}
    @php
        $routePrefix = Auth::user()->isSuperAdmin() ? 'super_admin.tracking.index' : 'admin.tracking.index';
    @endphp
    <div class="grid grid-cols-2 sm:grid-cols-5 gap-3">
        <a href="{{ route($routePrefix) }}" class="bg-white dark:bg-slate-800 p-4 rounded-xl border-2 {{ !request('status') ? 'border-indigo-500 bg-indigo-50/20' : 'border-slate-200 dark:border-slate-700' }} shadow-sm transition-all hover:shadow-md">
        ...
        <a href="{{ route($routePrefix, ['status' => 'pending']) }}" class="bg-white dark:bg-slate-800 p-4 rounded-xl border-2 {{ request('status') === 'pending' ? 'border-indigo-500 bg-indigo-50/20' : 'border-slate-200 dark:border-slate-700' }} shadow-sm transition-all hover:shadow-md">
        ...
        <a href="{{ route($routePrefix, ['status' => 'revision']) }}" class="bg-white dark:bg-slate-800 p-4 rounded-xl border-2 {{ request('status') === 'revision' ? 'border-rose-500 bg-rose-50/20' : 'border-slate-200 dark:border-slate-700' }} shadow-sm transition-all hover:shadow-md">
            <p class="text-[10px] font-extrabold uppercase tracking-wider text-rose-600">Revisi</p>
            <p class="text-2xl font-black text-rose-600 mt-1">{{ $stats['revision'] }}</p>
        </a>
        <a href="{{ route($routePrefix, ['status' => 'approved']) }}" class="bg-white dark:bg-slate-800 p-4 rounded-xl border-2 {{ request('status') === 'approved' ? 'border-emerald-500 bg-emerald-50/20' : 'border-slate-200 dark:border-slate-700' }} shadow-sm transition-all hover:shadow-md">
        ...
        <a href="{{ route($routePrefix, ['status' => 'rejected']) }}" class="bg-white dark:bg-slate-800 p-4 rounded-xl border-2 {{ request('status') === 'rejected' ? 'border-rose-500 bg-rose-50/20' : 'border-slate-200 dark:border-slate-700' }} shadow-sm transition-all hover:shadow-md col-span-2 sm:col-span-1">
    ...
    {{-- ── Filter Bar ── --}}
    <form method="GET" action="{{ route($routePrefix) }}" class="flex flex-col sm:flex-row gap-3 bg-white dark:bg-slate-800 p-4 rounded-xl border-2 border-slate-300 dark:border-slate-600 shadow-sm">
        {{-- Search --}}
        <div class="relative flex-1">
            ...
            <input type="text" name="search" value="{{ request('search') }}"
                   placeholder="Cari judul, nomor surat, atau pengaju..."
    ...
    <div class="space-y-6">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl border-2 border-slate-300 dark:border-slate-600 shadow-sm overflow-hidden p-5 sm:p-6 transition-all hover:border-indigo-400">
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <div class="flex items-center gap-2 flex-wrap">
                                @php
                                    $detailRoute = Auth::user()->isSuperAdmin() ? route('super_admin.tracking.show', $letter) : route('admin.tracking.show', $letter);
                                @endphp
                                <a href="{{ $detailRoute }}" class="text-lg font-black text-slate-900 dark:text-white hover:text-indigo-600 transition-colors">
                                    {{ $letter->title }}
                                </a>
                            ...
                            <div class="flex items-center gap-3 text-xs text-slate-500 mt-1 flex-wrap font-medium">
                                ...
                                <span class="text-slate-300">•</span>
                                <span class="font-bold text-slate-800 dark:text-slate-200">Pengaju: {{ $letter->creator->name ?? 'Staff' }}</span>
                                <span class="text-slate-300">•</span>
                                <span>{{ $letter->created_at->format('d M Y, H:i') }} WIB</span>
                            </div>
                        </div>
                    </div>

                    <div class="flex items-center gap-2 justify-end">
                        <a href="{{ $detailRoute }}" class="inline-flex items-center gap-1.5 px-4 py-2 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 border border-indigo-200 rounded-xl text-xs font-bold transition-colors shadow-sm">
                ...
                @if($latestLog && $latestLog->notes)
                    <div class="mt-2 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl p-3 flex items-start gap-3">
                        ...
                        <div class="text-xs">
                            <span class="font-extrabold text-slate-800 dark:text-slate-200">Catatan Aktivitas Terakhir ({{ $latestLog->user->name ?? 'Sistem' }} - {{ $latestLog->created_at->diffForHumans() }}):</span>
        ...
        @empty
            <div class="bg-white dark:bg-slate-800 rounded-2xl border-2 border-slate-300 dark:border-slate-600 shadow-sm p-12 text-center">
                ...
                <p class="text-slate-500 font-medium text-sm mt-1">Belum ada data surat pengajuan yang dapat ditampilkan sesuai dengan kriteria filter.</p>
            </div>
        @endforelse
    </div>
