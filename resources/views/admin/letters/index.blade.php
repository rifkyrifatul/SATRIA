...
<div class="max-w-7xl mx-auto space-y-6">
    ...
    <div class="mb-6">
        <h1 class="text-2xl font-extrabold text-slate-900 dark:text-white">Daftar Dokumen</h1>
        <p class="text-sm text-slate-500 mt-1">Kelola dan pantau seluruh surat masuk di dalam sistem.</p>
    </div>

    {{-- ── Filter Bar ── --}}
    <form method="GET" action="{{ Auth::user()->isSuperAdmin() ? route('super_admin.letters.index') : route('admin.letters.index') }}"
        ...
        {{-- Filter Status --}}
        <select name="status" onchange="this.form.submit()" class="w-full sm:w-48 py-2.5 px-4 text-sm border border-slate-300 dark:border-slate-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-slate-50 dark:bg-slate-900/50 text-slate-600 dark:text-slate-400 transition-colors">
            <option value="">Semua Status</option>
            <option value="pending" {{ request('status') == 'pending' ? 'selected' : '' }}>Menunggu ({{ $statusCounts['pending'] ?? 0 }})</option>
            <option value="revision" {{ request('status') == 'revision' ? 'selected' : '' }}>Revisi ({{ $statusCounts['revision'] ?? 0 }})</option>
            <option value="approved" {{ request('status') == 'approved' ? 'selected' : '' }}>Disetujui ({{ $statusCounts['approved'] ?? 0 }})</option>
            <option value="rejected" {{ request('status') == 'rejected' ? 'selected' : '' }}>Ditolak ({{ $statusCounts['rejected'] ?? 0 }})</option>
        </select>

        @if(request('search') || request('status'))
            <a href="{{ Auth::user()->isSuperAdmin() ? route('super_admin.letters.index') : route('admin.letters.index') }}" class="px-5 py-2.5 text-sm font-bold text-slate-600 dark:text-slate-400 hover:text-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 flex items-center shadow-sm transition-colors">Reset</a>
        @endif
        <button type="submit"
                        ...
                        <tr class="border-b border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-all duration-200 group last:border-0">
                            ...
                            <td class="px-6 py-4">
                                @if($letter->letter_number)
                                    <span class="text-[10px] bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 px-2 py-0.5 rounded-md font-mono font-bold border border-slate-200 dark:border-slate-700 shadow-sm whitespace-nowrap">{{ $letter->letter_number }}</span>
                                @else
                                    <span class="text-[10px] bg-slate-50 dark:bg-slate-800 text-slate-400 px-2 py-0.5 rounded-md italic border border-slate-100 dark:border-slate-700 whitespace-nowrap">Belum bernomor</span>
                                @endif
                            </td>
                            <td class="px-6 py-4">
                                @if($letter->category)
                                    <span class="text-[10px] bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 px-2 py-0.5 rounded-md font-bold whitespace-nowrap">{{ $letter->category->name }}</span>
                                @else
                            ...
                            <td class="px-6 py-4 hidden lg:table-cell">
                                <div class="flex items-start gap-2.5">
                                    <div class="w-7 h-7 rounded-lg bg-slate-50 dark:bg-slate-800 flex items-center justify-center border border-slate-100 dark:border-slate-700 flex-shrink-0">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <a href="{{ Auth::user()->isSuperAdmin() ? route('super_admin.letters.download', $letter) : route('admin.letters.download', $letter) }}"
                                       class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-500 hover:text-emerald-600 hover:bg-emerald-50 hover:border-emerald-200 dark:hover:bg-emerald-900/30 transition-all shadow-sm"
                                       title="Unduh Dokumen">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-6 py-20 text-center">
                                <div class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-slate-50 dark:bg-slate-900/50 mb-4 border border-slate-100">
                                    <svg class="w-8 h-8 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                              d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                                    </svg>
                                </div>
                                <p class="text-sm font-bold text-slate-700 dark:text-slate-300">Tidak ada surat ditemukan</p>
                                <p class="text-xs text-slate-500 mt-1">Coba ubah filter atau kata kunci pencarian Anda.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
