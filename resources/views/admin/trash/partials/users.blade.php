@if($users->isEmpty())
    <div class="flex flex-col items-center justify-center py-20 px-4 text-center">
        ...
        <p class="text-sm font-bold text-slate-700 dark:text-slate-300">Tong sampah pengguna kosong</p>
        <p class="text-xs font-medium text-slate-500 mt-1">Tidak ada pengguna yang dihapus saat ini.</p>
    </div>
@else
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-indigo-600 dark:bg-indigo-800">
                    ...
                    <th class="px-6 py-4 text-xs font-extrabold text-white uppercase tracking-wider">Pengguna</th>
                    <th class="px-6 py-4 text-xs font-extrabold text-white uppercase tracking-wider text-center">Peran</th>
                ...
                @foreach($users as $index => $user)
                    <tr class="hover:bg-slate-50/50 transition-colors group">
                        <td class="px-6 py-4 text-center">
                            <span class="text-sm font-bold text-slate-500">{{ $users->firstItem() + $index }}</span>
                        </td>
                        <td class="px-6 py-4">
                            <div class="text-sm font-bold text-slate-900 dark:text-white group-hover:text-indigo-600 transition-colors">{{ $user->name }}</div>
                            <div class="text-xs text-slate-500 mt-0.5">{{ $user->email }}</div>
                        </td>
                        <td class="px-6 py-4 text-center">
                            <span class="inline-flex items-center justify-center px-2 py-1 text-[10px] font-extrabold uppercase tracking-widest rounded-md bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300">{{ str_replace('_', ' ', $user->role) }}</span>
                        </td>
                        <td class="px-6 py-4 text-slate-500 text-xs font-medium">
                            <span class="font-bold text-slate-700 dark:text-slate-300">{{ $user->deleted_at->format('d M Y') }}</span><br>
                            <span class="text-[10px] text-slate-400 font-bold uppercase tracking-wider">{{ $user->deleted_at->format('H:i') }} WIB</span>
                        </td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <form action="{{ route('super_admin.trash.users.restore', $user->id) }}" method="POST">
                                    @csrf @method('PUT')
                                ...
                                <form action="{{ route('super_admin.trash.users.force_delete', $user->id) }}" method="POST" onsubmit="confirmDelete(event, 'Yakin hapus permanen?');">
                                    @csrf @method('DELETE')
