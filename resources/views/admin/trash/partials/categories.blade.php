                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors group">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <form action="{{ route(Auth::user()->isSuperAdmin() ? 'super_admin.trash.categories.restore' : 'admin.trash.categories.restore', $category->id) }}" method="POST">
                                    @csrf @method('PUT')
                                ...
                                <form action="{{ route(Auth::user()->isSuperAdmin() ? 'super_admin.trash.categories.force_delete' : 'admin.trash.categories.force_delete', $category->id) }}" method="POST" onsubmit="confirmDelete(event, 'Yakin hapus permanen?');">
                                    @csrf @method('DELETE')
