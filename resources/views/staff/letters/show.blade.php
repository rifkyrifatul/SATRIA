...
<div class="max-w-7xl mx-auto space-y-6" x-data="{ showPdfModal: false, pdfUrl: '', pdfTitle: '' }">
    ...
    <div class="flex items-center gap-4 mb-6">
        <a href="{{ route('staff.letters.index') }}" class="p-2.5 bg-white dark:bg-slate-800 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-slate-700/50 rounded-xl border-2 border-slate-200 dark:border-slate-700 transition-colors shadow-sm">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path>
            </svg>
        </a>
        <div>
            <h1 class="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Informasi Lengkap Surat</h1>
            <p class="text-sm font-medium text-slate-500 mt-1">Lihat detail surat, unduh dokumen, dan pantau riwayat pengajuan.</p>
        </div>
    </div>
        ...
        <div class="p-5 sm:p-6">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-5">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap items-center gap-3">
                            ...
                            @endif
                            <span class="text-xs font-bold text-slate-500 bg-slate-100 dark:bg-slate-700 px-2.5 py-1 rounded-md flex items-center gap-1.5 border border-slate-200 dark:border-slate-600">
                ...
                <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
                    @if($letter->isApproved() || $letter->isManualPending())
                        <a href="{{ route('staff.letters.print', $letter) }}" 
                           class="inline-flex justify-center items-center gap-2 px-5 py-2.5 text-xs uppercase tracking-wider font-bold bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-colors shadow-sm">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
                            Unduh Dokumen
                        </a>
                    @elseif($letter->isRevision())
                        <a href="{{ route('staff.letters.edit', $letter) }}" 
                           class="inline-flex justify-center items-center gap-2 px-5 py-2.5 text-xs uppercase tracking-wider font-bold bg-amber-500 text-white rounded-xl hover:bg-amber-600 transition-colors shadow-sm">
                    ...
                    @if($letter->attachments->isEmpty())
                        ...
                        <div class="flex flex-col gap-2">
                            @if(strtolower($letter->file_extension) === 'pdf')
                                <button @click.prevent="pdfUrl = '{{ route('staff.letters.preview', $letter) }}'; pdfTitle = 'Dokumen Aktif'; showPdfModal = true" class="inline-flex justify-center items-center gap-2 w-full px-4 py-2 text-xs font-bold bg-indigo-100 text-indigo-700 rounded-lg hover:bg-indigo-200 transition-colors shadow-sm">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                    Lihat Dokumen
                                </button>
                            @endif
                            <a href="{{ route('staff.letters.download', $letter) }}" class="inline-flex justify-center items-center gap-2 w-full px-4 py-2 text-xs font-bold bg-slate-900 text-white rounded-lg hover:bg-slate-800 transition-colors shadow-sm">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
                                Unduh Aktif
                            </a>
                        </div>
                                    ...
                                    <div class="flex items-start gap-3">
                                        ...
                                        <div class="flex-1">
                                            ...
                                            <p class="text-[10px] font-bold text-slate-400">
                                                Oleh: {{ $attachment->user->name ?? 'Sistem' }}
                                            </p>
                                        </div>
                                    </div>
                                    <div class="flex gap-2">
                                        @if(strtolower(pathinfo($attachment->file_path, PATHINFO_EXTENSION)) === 'pdf')
                                            <button @click.prevent="pdfUrl = '{{ route('staff.letters.preview', ['letter' => $letter->id, 'attachment_id' => $attachment->id]) }}'; pdfTitle = '{{ $typeLabel }}'; showPdfModal = true" class="flex-1 inline-flex justify-center px-4 py-1.5 text-xs font-bold border-2 border-indigo-200 text-indigo-700 hover:bg-indigo-50 dark:hover:bg-indigo-900/50 rounded-xl transition-colors bg-white dark:bg-slate-800 flex items-center gap-2">
                                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                                Lihat
                                            </button>
                                        @endif
                                        <a href="{{ route('staff.letters.download', ['letter' => $letter->id, 'attachment_id' => $attachment->id]) }}" 
                                           class="flex-1 inline-flex justify-center px-4 py-1.5 text-xs font-bold border-2 {{ $isLatest ? 'border-indigo-600 text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/50' : 'border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700' }} rounded-xl transition-colors bg-white dark:bg-slate-800 flex items-center gap-2">
    ...
    <div x-show="showPdfModal" 
         class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/80 backdrop-blur-sm"
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-5xl h-[90vh] flex flex-col shadow-2xl overflow-hidden mx-4" @click.away="showPdfModal = false">
            ...
            <div class="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50">
                ...
                <button @click="showPdfModal = false" class="text-slate-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 p-2 rounded-xl transition-colors focus:outline-none">
            ...
            <div class="flex-1 bg-slate-100 dark:bg-slate-900 relative">
                <div class="absolute inset-0 flex items-center justify-center" x-show="!pdfUrl">
