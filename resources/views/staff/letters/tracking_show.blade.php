...
<div class="max-w-7xl mx-auto space-y-6" x-data="{ showPdfModal: false, pdfUrl: '', pdfTitle: '' }">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div class="flex items-center gap-4">
            <a href="{{ route('staff.letters.index') }}" class="p-2.5 bg-white dark:bg-slate-800 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-slate-700/50 rounded-xl border-2 border-slate-200 dark:border-slate-700 transition-colors shadow-sm" title="Kembali ke Daftar Surat">
            ...
            <div>
                <h1 class="text-2xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-2">
                    ...
                    Detail Tracking Posisi Surat
                </h1>
                <p class="text-sm font-medium text-slate-500 mt-1">Pantau posisi tahapan verifikasi, unduh berkas, dan lihat riwayat aktivitas surat ini.</p>
            </div>
        </div>

        @if(isset($allLetters) && $allLetters->count() > 1)
            <div class="flex items-center gap-2">
                <label for="letterSelect" class="text-xs font-bold text-slate-500 dark:text-slate-400 whitespace-nowrap">Pilih Surat:</label>
                <select id="letterSelect" onchange="if(this.value) window.location.href=this.value" 
                    ...
                    @foreach($allLetters as $item)
                        <option value="{{ route('staff.tracking.show', $item) }}" {{ $item->id === $letter->id ? 'selected' : '' }}>
                                ...
                                <div class="flex flex-col gap-2.5 p-3.5 rounded-xl border-2 {{ $isLatest ? 'border-indigo-200 dark:border-indigo-800 bg-indigo-50/50 dark:bg-indigo-900/20' : 'border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50' }}">
                                    ...
                                    <div class="flex items-center gap-2 mt-1">
                                        @if(strtolower(pathinfo($attachment->file_path, PATHINFO_EXTENSION)) === 'pdf')
                                            <button @click.prevent="pdfUrl = '{{ route('staff.letters.preview', ['letter' => $letter->id, 'attachment_id' => $attachment->id]) }}'; pdfTitle = '{{ $typeLabel }}'; showPdfModal = true"
                                        ...
                                        @endif
                                        <a href="{{ route('staff.letters.download', ['letter' => $letter->id, 'attachment_id' => $attachment->id]) }}" class="inline-flex justify-center items-center gap-1 px-3 py-1.5 rounded-lg bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 text-xs font-bold transition-colors shadow-sm flex-1">
